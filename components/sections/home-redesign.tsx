...
const partnerLogos = [
  { name: "SBI", src: "/images/partners/SBI.png" },
  { name: "ICICI Bank", src: "/images/partners/ICICI.png" },
  { name: "HDFC Bank", src: "/images/partners/HDFC.png" },
  { name: "Axis Bank", src: "/images/partners/AXIS.png" },
  { name: "Airtel Payments Bank", src: "/images/partners/Airtel.png" },
  { name: "Paytm Payments Bank", src: "/images/partners/paytm.png" },
  { name: "Kotak", src: "/images/partners/kotak.png" },
  { name: "IDFC FIRST Bank", src: "/images/partners/IDFC.png" },
] as const;

const heroMetrics = [
  { icon: UserPlus, value: "10,000+", label: "Retailer Partners", copy: "Across India" },
  { icon: Landmark, value: "500+", label: "Districts Covered", copy: "And Growing" },
  { icon: CircleDollarSign, value: "1 Cr+", label: "Transactions", copy: "Every Month" },
  { icon: ShieldCheck, value: "99.9%", label: "Transaction", copy: "Success Rate" },
  { icon: Headphones, value: "24x7", label: "Support", copy: "Always Here" },
] as const;
      ...
      <section className="home-reference-hero relative overflow-hidden border-b border-[#10213f]/[.05] dark:border-white/[.05]">
        ...
        <div className="relative mx-auto w-full max-w-[1260px] px-4 pb-9 pt-10 sm:px-6 sm:pb-12 sm:pt-12 lg:px-8 lg:pt-14">
          <div className="grid items-center gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-8">
            <motion.div {...fadeUp} className="relative z-10 max-w-[540px]">
              <span className="inline-flex rounded-full border border-[#11995a]/20 bg-[#11995a]/[.07] px-3 py-1.5 text-[10px] font-bold text-[#10864f] dark:border-[#54e394]/20 dark:bg-[#54e394]/[.08] dark:text-[#75efaa]">
                One Platform, Every Financial Service.
              </span>
              <h1 className="mt-5 font-display text-[clamp(2.55rem,4.6vw,4.35rem)] font-semibold leading-[1.03] tracking-[-0.045em] text-[#10213f] dark:text-white">
                Bringing Bills and
              ...
              <div className="mt-7 grid max-w-[455px] grid-cols-3 gap-3 sm:gap-5">
            ...
            >
              <div className="hero-dashboard-vignette relative mx-auto aspect-[1.5/1] w-full max-w-[720px]">
                <Image
                  src="/images/home/34.png"
                  alt="billbring financial services ecosystem"
                  fill
                  priority
          ...
          <motion.div {...fadeUp} className="relative z-10 mt-7 overflow-hidden rounded-[14px] border border-[#10213f]/[.075] bg-white/95 shadow-[0_22px_60px_-44px_rgba(16,33,63,.45)] backdrop-blur dark:border-white/[.08] dark:bg-[#0b1622]/95 lg:mt-4">
            <p className="border-b border-[#10213f]/[.06] py-2 text-center text-[9px] font-extrabold text-[#41516a] dark:border-white/[.07] dark:text-slate-300">All Financial Services in One Platform</p>
            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-12">
              {services.map(({ title, copy, icon: Icon, tone }) => {
                const toneClass = tone === "green" ? "text-[#11995a]" : tone === "orange" ? "text-[#ee9b3b]" : "text-[#3068d8]";
                return (
                  <div key={`hero-${title}`} className="flex min-h-[88px] flex-col items-center justify-center border-b border-r border-[#10213f]/[.055] px-2 py-3 text-center last:border-r-0 dark:border-white/[.055] lg:border-b-0">
                    <Icon className={`h-4 w-4 ${toneClass}`} />
                    <strong className="mt-2 text-[8px] font-extrabold leading-3 text-[#10213f] dark:text-white sm:text-[9px]">{title}</strong>
                    <span className="mt-0.5 line-clamp-2 text-[7px] leading-[10px] text-[#6f7b8f] dark:text-slate-400">{copy}</span>
                  </div>
                );
              })}
            </div>
          ...
          <motion.div {...fadeUp} className="relative z-10 mt-4 grid overflow-hidden rounded-[12px] border border-[#11995a]/[.12] bg-[#f6fcf9]/95 shadow-[0_18px_45px_-40px_rgba(17,153,90,.5)] backdrop-blur dark:border-[#54e394]/[.10] dark:bg-[#091a16]/90 sm:grid-cols-5">
            {heroMetrics.map(({ icon: Icon, value, label, copy }) => (
              <div key={value + label} className="flex min-h-[72px] items-center justify-center gap-3 border-b border-[#11995a]/[.09] px-4 py-3 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0 dark:border-[#54e394]/[.08]">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#11995a]/[.08] text-[#11995a] dark:bg-[#54e394]/[.08] dark:text-[#54e394]"><Icon className="h-5 w-5" /></span>
                <span>
                  <strong className="block text-[16px] font-black leading-5 text-[#10213f] dark:text-white">{value}</strong>
                  <span className="block text-[8px] font-bold leading-3 text-[#45546b] dark:text-slate-300">{label}</span>
                  <span className="block text-[7px] leading-3 text-[#7a8798] dark:text-slate-500">{copy}</span>
                </span>
              </div>
            ))}
          </motion.div>
            ...
            <div className="grid items-center gap-8 lg:grid-cols-[0.82fr_1.18fr]">
              ...
              <div>
                ...
                <div className="relative mt-6 aspect-[1.55/1] w-full max-w-[620px]">
                  <Image src="/images/home/2.png" alt="billbring business dashboard analytics" fill sizes="(max-width: 1024px) 100vw, 50vw" className="dashboard-feature-image object-cover object-center" />
                </div>
              </div>
            </div>
          </motion.div>
        ...
        <div className="mx-auto w-full max-w-[1260px] px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">
            {partnerLogos.map((partner) => (
              <div key={partner.name} className="grid min-h-[74px] place-items-center rounded-[8px] border border-[#10213f]/[.07] bg-white px-4 py-3 shadow-[0_10px_24px_-24px_rgba(16,33,63,.4)] dark:border-white/[.07] dark:bg-[#0b1622]">
                <div className="relative h-9 w-full">
                  <Image src={partner.src} alt={`${partner.name} logo`} fill sizes="150px" className="object-contain" />
                </div>
              </div>
            ))}
          </div>
